import { ZeroAddress, EventLog, BigNumberish, ethers, formatEther } from "ethers";
import { BaseTransaction } from '@safe-global/safe-apps-sdk';
import { PluginMetadata, loadPluginMetadata } from "./metadata";
import { getManager, getPlugin, getRegistry, getSafeStreet, getSafeStreetToken } from "./protocol";
...
const PUBLISHER_INFO = { '0xaA498424C846c44e2029E1835f9549d86d7C5E44' : {
    logo: '',
...
export const publishWithPayment = async (plugin: string, token: string, amount: string) => {

    const provider =  new ethers.BrowserProvider(window.ethereum)
    const safeStreet = await getSafeStreet(await provider.getSigner())
    const publishTx = await safeStreet.publishWithPayment(plugin, token, ethers.parseEther(amount))
...
export const fullfillePayment = async (plugin: string, token: string, amount: string ) => {
    ...
    const tokenAllowance = ethers.formatEther(await safeStreetToken.allowance(safeInfo.safeAddress, safeStreet))
    ...
    txs.push({
        ...
        value: "0",
        data: (await safeStreetToken.approve.populateTransaction(await safeStreet.getAddress(), tokenBalance)).data
    })
    }
    ...
    txs.push({
        ...
        data: (await safeStreet.fullfillPayment.populateTransaction(plugin, safeInfo.safeAddress)).data
    })
