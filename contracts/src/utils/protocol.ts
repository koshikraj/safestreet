...
export const getProtocolManagerAddress = async (hre: HardhatRuntimeEnvironment): Promise<string> => {
    ...
    const manager = (protocolDeployments as any)[chainId][0].contracts.SafeProtocolManagerWrapper.address;
...
export const getProtocolRegistryAddress = async (hre: HardhatRuntimeEnvironment): Promise<string> => {
    ...
    const registry = (protocolDeployments as any)[chainId][0].contracts.SafeProtocolRegistryWrapper.address;
    if (typeof registry !== "string") throw Error("Unexpected Registry");
    return registry;
};

export const getSafeStreetAddress = async (hre: HardhatRuntimeEnvironment): Promise<string> => {
    const chainId = await hre.getChainId();
    ...
    if (!(chainId in protocolDeployments)) throw Error("Unsupported Chain");
    // We use the unrestricted registry for the demo
    const registry = (protocolDeployments as any)[chainId][0].contracts.SafeStreetWrapper.address;
    if (typeof registry !== "string") throw Error("Unexpected Registry");
    return registry;
};
